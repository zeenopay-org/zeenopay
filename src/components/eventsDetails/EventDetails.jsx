...
function EventDetails() {
  ...
  return (
    <div className="bg-customBlue max-w-full py-8 px-4 flex flex-col items-center pb-20 min-h-screen">
      <>
        {pop && (
          <QrCard handleX={handleX} qrid={qrId} />
        )}

        <div className={`w-full ${pop ? "blur-md pointer-events-none" : ""} flex justify-center items-center w-full`}>
          {loading || skeletonDelay ? (
            <div className="w-full max-w-[90%] h-[300px] md:h-[500px] bg-gray-600 animate-pulse rounded-2xl mb-6"></div>
          ) : (
            <div className="relative flex flex-col w-full h-full items-center justify-center">
              <img
                src={event.img}
                alt={event.title}
                className="w-full max-w-[90%] h-[300px] md:h-[500px] object-cover rounded-2xl mb-6"
                width="1080"
                height="500"
                loading="lazy"
                decoding="async"
              ...
              {event.misc_kv ? (
                <img
                  src={event.misc_kv}
                  alt=""
                  className="absolute -bottom-10 md:-bottom-4 left-1/2 transform -translate-x-1/2 h-28 w-28 md:h-40 md:w-40 rounded-full border-4 border-white object-cover"
                  width="160"
                  height="160"
                />
              ) : null}
        ...
        <div className={`w-full ${pop ? "blur-md pointer-events-none" : ""} grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-5 w-full max-w-[90%]`}>
          {loading
            ? Array(6)
                .fill()
                .map((_, index) => (
