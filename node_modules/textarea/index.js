...
exports.measureLeadingWhitespace = measureLeadingWhitespace;
/**
Remove leading whitespace from each line of the input up to the specified amount.
*/
function dedent(input, spaces) {
    if (spaces === void 0) { spaces = measureLeadingWhitespace(input); }
...
function escapeHTML(html) {
    ...
    // '\n': '<br/>' /[\n]/g
    return html && html.replace(/[&<>"]/g, function (entity) { return htmlReplacements[entity]; });
}
function copyStyleSubset(source, target, properties) {
    var cssStyleDeclaration = getComputedStyle(source);
    properties.forEach(function (property) {
        var sourceValue = cssStyleDeclaration.getPropertyValue(property);
        target.style.setProperty(property, sourceValue);
    });
}
/** A (the?) subset of styles that are relevant to the painted size of an
in-flow element */
var cssSizeProperties = [
    // padding
...
function dentSelection(textarea, dedent, tab) {
    ...
    if (dedent) {
        ...
        var dedentRegExp = new RegExp((begin === 0) ? '(^|\n)' + tab : '(\n)' + tab, 'g');
        // removing a tab
        middle = middle.replace(dedentRegExp, '$1');
    }
    else {
        var indentRegExp = new RegExp((begin === 0) ? '(^|\n)' : '(\n)', 'g');
        // indenting a tab
...
function autoindentNewline(textarea, jump) {
    ...
    var previousIndentMatch = value.slice(beginning_of_previous_line).match(/^[ \t]+/);
...
var Textarea = (function () {
    ...
    */
    function Textarea(el, _a) {
        var _b = _a === void 0 ? {} : _a, _c = _b.tab, tab = _c === void 0 ? '  ' : _c, _d = _b.buffer, buffer = _d === void 0 ? 0 : _d, _e = _b.autodedent, autodedent = _e === void 0 ? false : _e, _f = _b.poll, poll = _f === void 0 ? null : _f;
        this.el = el;
    ...
    Textarea.prototype.initTabListener = function () {
        ...
        this.el.addEventListener('keydown', function (ev) {
            ...
            if (ev.which == 9) {
                ...
                dentSelection(_this.el, ev.shiftKey, _this.indentContent);
            }
        }, false);
    };
    /**
    ...
    */
    Textarea.prototype.initNewlineListener = function () {
        var _this = this;
        this.el.addEventListener('keydown', function (ev) {
            if (ev.which == 13) {
                var handled = autoindentNewline(_this.el, ev.metaKey);
                if (handled) {
    ...
    */
    ...
    Textarea.prototype.resizeToFit = function () {
        ...
        var shadowStyle = getComputedStyle(this.shadow);
        var shadowHeight = parseInt(shadowStyle.height, 10);
